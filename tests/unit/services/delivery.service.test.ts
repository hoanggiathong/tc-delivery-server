...
describe('DeliveryService', () => {
  ...
  describe('createDelivery', () => {
    ...
    it('should throw error when from route not found', async () => {
      ...
      await expect(deliveryService.createDelivery(mockDeliveryData, 'user123')).rejects.toThrow(
  ...
  describe('deleteDelivery', () => {
    ...
    it('should throw error when delete fails', async () => {
      ...
      await expect(deliveryService.deleteDelivery(mockDeliveryId)).rejects.toThrow('Delete error');
    });
  });
  ...
  describe('getDeliveryByCode', () => {
    ...
    it('should throw error for invalid delivery identifier format', async () => {
      await expect(deliveryService.getDeliveryByCode('invalid')).rejects.toThrow(
        'Invalid delivery identifier format'
      );
    });
    ...
    it('should throw error when from route not found', async () => {
      ...
      // Mock Route.findOne calls
      MockedRoute.findOne = jest.fn().mockResolvedValueOnce(null); // From route not found

      await expect(deliveryService.getDeliveryByCode('2401250001T1T2')).rejects.toThrow(
        'From route with code T1 not found'
      );
    });
    ...
    it('should throw error when to route not found', async () => {
      ...
      await expect(deliveryService.getDeliveryByCode('2401250001T1T2')).rejects.toThrow(
        'To route with code T2 not found'
      );
    });
